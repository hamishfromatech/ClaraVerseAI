...
// Storage key names
const SESSION_STORAGE_KEY = 'atech-secure-keys';
const LOCAL_STORAGE_KEY = 'atech-secure-keys-persist';
...
export const useSecureKeyStore = create<SecureKeyState>()((set, get) => ({
  ...
  initialize: async () => {
    ...
    try {
      ...
      const persistFlag = localStorage.getItem('atech-persist-keys');
  ...
  setPersistKeys: async (persist: boolean) => {
    ...
    // Save preference
    localStorage.setItem('atech-persist-keys', String(persist));
